...
//
//  Created by Affluxis on Jan 25, 2024.
//  Copyright © 2024 Affluxis. All rights reserved.
//
...
// https://github.com/ggerganov/whisper.cpp/issues/878
// https://github.com/Const-me/Whisper
// https://github.com/ggerganov/whisper.cpp/issues/888
...
bool whisper_params_parse(int argc, char** argv, whisper_params& params) {
	for (int i = 1; i < argc; i++) {
		...
		if (arg == "-h" || arg == "--help") {
			...
			exit(0);
		} else if (arg == "-t" || arg == "--threads") { params.n_threads = stoi(argv[++i]); } else if (arg == "--step") { params.step_ms = stoi(argv[++i]); } else if (arg == "--length") { params.length_ms = stoi(argv[++i]); } else if (arg == "--keep") { params.keep_ms = stoi(argv[++i]); } else if (arg == "-c" || arg == "--capture") { params.capture_id = stoi(argv[++i]); } else if (arg == "-mt" || arg == "--max-tokens") { params.max_tokens = stoi(argv[++i]); } else if (arg == "-ac" || arg == "--audio-ctx") { params.audio_ctx = stoi(argv[++i]); } else if (arg == "-vth" || arg == "--vad-thold") { params.vad_thold = stof(argv[++i]); } else if (arg == "-fth" || arg == "--freq-thold") { params.freq_thold = stof(argv[++i]); } else if (arg == "-tr" || arg == "--translate") { params.translate = true; } else if (arg == "-nf" || arg == "--no-fallback") { params.no_fallback = true; } else if (arg == "-ps" || arg == "--print-special") { params.print_special = true; } else if (arg == "-kc" || arg == "--keep-context") { params.no_context = false; } else if (arg == "-l" || arg == "--language") { params.language = argv[++i]; } else if (arg == "-m" || arg == "--model") { params.model = argv[++i]; } else if (arg == "-f" || arg == "--file") { params.fname_out = argv[++i]; } else if (arg == "-tdrz" || arg == "--tinydiarize") { params.tinydiarize = true; } else if (arg == "-sa" || arg == "--save-audio") { params.save_audio = true; } else if (arg == "-ng" || arg == "--no-gpu") { params.use_gpu = false; } else if (arg == "-fa" || arg == "--flash-attn") { params.flash_attn = true; }
		...
		// run the inference
		{
			...
			// print result;
			{
				if (!use_vad) {
					...
					printf("%s", string(100, ' ').c_str());

					printf("\33[2K\r");
				} else {
					const int64_t t1 = (t_last - t_start).count() / 1000000;
					const int64_t t0 = max(0.0, t1 - pcmf32.size() * 1000.0 / WHISPER_SAMPLE_RATE);
				...
				for (int i = 0; i < n_segments; ++i) {
					...
					if (params.no_timestamps) {
						...
						string output = "[" + to_timestamp(t0, false) + " --> " + to_timestamp(t1, false) + "]  " + text;
